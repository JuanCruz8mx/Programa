...
class main():
    def __init__(self):
        self.G=Nx.Graph()
        self.v=Tk.Tk()
        self.v.geometry('1300x650+300+200')
        self.v.title('RouteLigic')
        self.v.config(bg='#404040')
        ...
        self.v.mainloop()
    def ventana(self):
        frame=Tk.Frame(self.v, bg='#404040'); frame.grid(row=0, column=0,sticky='N',padx=1,pady=1)
        ...
        self.canvas = Tk.Canvas(self.frame3, width=800, height=600, bg='#f8f8f8')
        ...
        origen=Tk.Label(frame, text='Origen: ', font=('arial', 15), bg='#404040'); origen.grid(row=0, column=1, padx=2, pady=2, sticky='E')
        ...
        i_name.config(font=('arial',15), width=15, bg='#722F37'); i_name.grid(row=0, column=2, padx=2, pady=2, sticky='NSEW')
        
        destino=Tk.Label(frame, text='Destino: ', font=('arial', 15), bg='#404040'); destino.grid(row=1, column=1, padx=2, pady=2,sticky='E')
        destino_var = Tk.StringVar()
        destino_var.set(opciones[0])
        i2_name = Tk.OptionMenu(frame, destino_var, *opciones)
        i2_name.config(font=('arial',15), width=15, bg='#722F37'); i2_name.grid(row=1, column=2, padx=2, pady=2, sticky='NSEW')

        boton = Tk.Button(frame, text='Mejor ruta', font=('arial',15), bg='#722F37', command=lambda: self.Grafo_ruta(origen_var.get(), destino_var.get())); boton.grid(row=3, column=2, padx=2, pady=2)

        self.respuesta=ScrolledText(frame2, width=40, height=20, wrap='word',font=('arial',15), bg='#f8f8f8'); self.respuesta.grid(row=3,column=1, columnspan=3,padx=2,pady=2,sticky='NSEW')
        self.mostrar_imagen('grafoCompleto.png')

    def mostrar_imagen(self, ruta):
        imagen_original = Image.open(ruta)
        self.imagen = ImageTk.PhotoImage(imagen_original.resize((800, 600)))
    ...
    def csv(self):
        ...
        with open('centros.csv', 'r') as archivo:
            ...
            for i in f[1:]:     # split(','): Divide cada dato separados por comas y los guarda en una lista
                N1, N2, Distancia, Capacidad, Tiempo, self.Trasporte=[x.strip() for x in i.split(',')[:6]] # Da el valor a cada uno de las variables
                #print(N1)
                if N1 not in self.nodos:
                    self.G.add_node(N1, distancia=Distancia, capacidad=Capacidad, trasporte=self.Trasporte)    # Crea el N1
                    self.nodos.add(N1)
                if N2 not in self.nodos:
                    self.G.add_node(N2, distancia=Distancia, tiempo=Tiempo,trasporte=self.Trasporte)      # Crea el N2
                    self.nodos.add(N2)
                self.G.add_edge(N1, N2, distancia=float(Distancia), tiempo=float(Tiempo), trasporte=str(self.Trasporte))     # Hace el grafo dirigido
    ...
    def Grafo(self):
        Nx.draw(self.G, self.pos, with_labels=True, node_color='lightblue', edge_color='black',node_size=60, font_size=10)   # Diseño del grafo
        #Nx.draw_networkx_edge_labels(self.G, self.pos, edge_labels=self.etiquetas, font_size=8) # Ver las etiquetas en el grafo
        plt.savefig('grafoCompleto.png')  # Guardar la imagen
        plt.close()
    def Ruta(self, ruta):
        r=list(zip(ruta[:-1], ruta[1:]))
        Nx.draw(self.G, self.pos, with_labels=True, node_color='lightgray', edge_color='lightgray',node_size=60, font_size=10)  # Diseño de los nodes y aristas completo
        Nx.draw_networkx_edges(self.G, self.pos, edgelist=r, edge_color='red', width=2, arrows=True)    # Diseño de los nodos de la ruta
        Nx.draw_networkx_nodes(self.G, self.pos, nodelist=ruta, node_color='red')     # Diseño de las aristas de la ruta
        #Nx.draw_networkx_edge_labels(self.G, self.pos, edge_labels=self.etiquetas, font_size=8)
    ...
    def Grafo_ruta(self, origen, destino):
        if origen in self.G.nodes and destino in self.G.nodes:
            if Nx.has_path(self.G, origen, destino):     # Revisa si existe una ruta
                ...
                self.respuesta.insert('insert', f'Tipo de trasporte: {self.Trasporte}\n')
